...
use crate::parse::guile::{
    scm_assq, scm_c_eval_string, scm_car, scm_cdr, scm_from_utf8_symbol, scm_is_bool, scm_is_false,
    scm_is_integer, scm_is_null, scm_is_real, scm_is_symbol, scm_is_true, scm_list_p,
    scm_object_to_string, scm_pair_p, scm_string_p, scm_symbol_to_string, scm_to_bool,
    scm_to_double, scm_to_int64, scm_to_utf8_stringn, scm_with_guile, wrapper_free, SCM,
};
...
pub(crate) fn with_guile<P: ?Sized, R>(func: extern "C" fn(&P) -> R, params: &P) -> R {
    ...
    //   unsafe. A nonlocal exit is detected by executing the actual user code
    //   inside a wrapper function, which either sets a result variable
    //   (CallInfo::res) if the code ran successfully, or does not set a result, if
    //   a nonlocal exit back up to the enclosing scm_with_guile call was done. In
    //   the latter case, we simply panic, because we just leaked a bunch of memory
    //   by subverting Rust's borrow checker with a longjmp. User code must ensure
    //   that this never happens.
    // - Guile initialization is not actually thread-safe (at least the
    //   initialization of any thread after the first one isn't). So the code before
    ...
    // Lock during Guile initialization, because something in there is not
    // thread-safe.
    ...
    unsafe {
        scm_with_guile(
            Some(wrapper::<P, R>),
            &call as *const CallInfo<P, R> as *mut c_void,
        );
        match call.res {
            Some(v) => v,
...
pub fn form_to_string(obj: SCM) -> String {
    unsafe {
        let scm_string = scm_object_to_string(
            obj,
            scm_c_eval_string(CStr::from_bytes_with_nul(b"write\0").unwrap().as_ptr()),
        );
...
mod tests {
    ...
    use crate::parse::guile::{scm_car, scm_from_int16, scm_from_int32, scm_to_int32};
    use crate::parse::scheme_util::{assq_str, eval_str};
