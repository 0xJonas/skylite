...
use super::guile::{scm_car, scm_cdr, scm_is_false, scm_list_p, SCM};
use super::project::AssetGroup;
use super::scheme_util::{assq_str, form_to_string, parse_symbol};
use super::values::{parse_argument_list, TypedValue, Variable};
use crate::parse::guile::scm_pair_p;
use crate::parse::scheme_util::{eval_str, iter_list, with_guile};
use crate::parse::util::{change_case, IdentCase};
use crate::parse::values::parse_variable_definition;
        ...
        unsafe {
            if scm_is_false(scm_list_p(form)) {
                return Err(SkyliteProcError::DataError(format!(
                    "Expected list for actor instantiation, got {}",
                    form_to_string(form)
                )));
            }
            ...
            Ok(ActorInstance { actor_name, args })
        }
    }
}
                    ...
                    .map(|e| {
                        if scm_is_false(scm_pair_p(e)) {
                            Err(SkyliteProcError::DataError(format!(
                                "Expected pair (name . instance) for actor, got {}",
                                form_to_string(e)
                            )))
                        } else {
                            Ok((
                                parse_symbol(scm_car(e))?,
                                ActorInstance::from_scheme(scm_cdr(e), actors)?,
                            ))
                        }
                    })
    ...
    pub(crate) fn from_file(path: &Path, actors: &[Actor]) -> Result<Scene, SkyliteProcError> {
        ...
        extern "C" fn from_file_guile(
            ...
            let definition_raw = read_to_string(path).map_err(|e| {
                SkyliteProcError::OtherError(format!("Error reading scene definition: {}", e))
            })?;
            let definition = unsafe { eval_str(&definition_raw)? };

            let name = change_case(
...
impl SceneInstance {
    pub(crate) fn from_scheme(
        def: SCM,
        scene_assets: &AssetGroup,
    ) -> Result<SceneInstance, SkyliteProcError> {
        unsafe {
            if scm_is_false(scm_list_p(def)) {
                return Err(SkyliteProcError::DataError(format!(
                    "Expected list for scene instantiation, got {}",
                    form_to_string(def)
                )));
            }
    ...
    pub(crate) fn _from_scheme_with_scenes(
        def: SCM,
        scenes: &[Scene],
    ) -> Result<SceneInstance, SkyliteProcError> {
        unsafe {
            ...
            Scene {
                ...
                actors: vec![
                    (
                        "a1".to_owned(),
                        ActorInstance {
                            actor_name: "TestActor".to_owned(),
                            args: vec![TypedValue::U8(1)]
                        }
                    ),
                    (
                        "a2".to_owned(),
                ...
                extras: vec![
                    ActorInstance {
                        actor_name: "TestActor".to_owned(),
                        args: vec![TypedValue::U8(3)]
                    },
                    ActorInstance {
                        actor_name: "TestActor".to_owned(),
                        args: vec![TypedValue::U8(4)]
                    },
                ],
