...
use super::guile::{scm_car, scm_cdr, scm_is_false, scm_is_null, scm_list_p, scm_pair_p, SCM};
use super::scheme_util::{assq_str, form_to_string, iter_list, parse_string};
use super::values::{parse_argument_list, parse_variable_definition, TypedValue, Variable};
use crate::parse::scheme_util::{eval_str, parse_symbol, with_guile};
        ...
        unsafe {
            if scm_is_false(scm_list_p(def)) && !scm_is_null(def) {
                return Err(SkyliteProcError::DataError(format!(
                    "Expected list for action definition, got {}",
                    form_to_string(def)
                )));
            }
    ...
    pub fn from_scheme(def: SCM, actions: &[Action]) -> Result<ActionInstance, SkyliteProcError> {
        unsafe {
            if scm_is_false(scm_pair_p(def)) && !scm_is_null(def) {
                return Err(SkyliteProcError::DataError(format!(
                    "Expected list for action instantiation, got {}",
                    form_to_string(def)
                )));
            }
    ...
    pub fn from_scheme(def: SCM, name: &str) -> Result<Actor, SkyliteProcError> {
        unsafe {
            ...
            let actions = if let Some(cs) = maybe_actions {
                ...
                return Err(SkyliteProcError::DataError(format!(
                    "Actor must contain at least one action"
                )));
            };
            ...
            let initial_action = if let Some(action) = maybe_initial_action {
                ...
                return Err(SkyliteProcError::DataError(format!(
                    "Missing required field 'initial-action'"
                )));
            };
...
mod tests {
    use super::Actor;
    use crate::parse::actors::{Action, ActionInstance, TypedValue};
