...
fn gen_properties_type(node: &Node, items: &[Item]) -> Result<TokenStream, SkyliteProcError> {
    ...
    let asset_properties = generate_member_list(&node.properties);
...
fn gen_static_nodes_type(node: &Node) -> TokenStream {
    ...
    quote! {
        struct #static_nodes_type_name {
            #(#members),*
        }
    }
}

fn gen_node_type(node: &Node) -> TokenStream {
    ...
    quote! {
        pub struct #node_name {
            ...
            pub dynamic_nodes: Vec<Box<dyn ::skylite_core::nodes::Node>>
        }
    }
}

fn gen_node_new_fn(node: &Node, items: &[Item]) -> TokenStream {
    let node_name = format_ident!("{}", change_case(&node.name, IdentCase::UpperCamelCase));
    let node_param_list = generate_param_list(&node.parameters);
    let node_args = generate_argument_list(&node.parameters);
    let properties_type_name = properties_type_name(&node.name);
...
fn gen_node_impl(
    ...
    let static_node_names: Vec<Ident> = node
        .static_nodes
        .keys()
        .map(|n| format_ident!("{}", change_case(n, IdentCase::LowerSnakeCase)))
        .collect();
    ...
    let z_order_call = get_annotated_function(items, "skylite_proc::z_order")
        .map(get_fn_name)
        .map_or(quote!(1), |item| quote!(#item(self, ctx)));
...
pub(crate) fn generate_node_definition(
    ...
    let node_type = gen_node_type(node);
    let node_new_fn = gen_node_new_fn(node, items);
    ...
    Ok(quote! {
        mod #node_module_name {
            mod gen {
                ...
                impl ::skylite_core::nodes::TypeId for #node_type {
                    fn get_id() -> usize {
                        #node_id
                    }
                }
...
mod tests {
    ...
    fn test_node_new_fn() {
        ...
        let actual = gen_node_new_fn(&node, &items);
        let expected = quote! {
