...
/// Generates a `TokenStream` of the form `var1: type1, var2: type2:, ...` from a list of `Variables`, used shared types.
/// Can be used for parameter lists.
pub(crate) fn generate_param_list(params: &[Variable]) -> TokenStream {
    ...
    let param_types = params.iter().map(|p| skylite_type_to_rust_param(&p.typename));
    quote! {
        #(#param_names: #param_types),*
    }
}

/// Generates a `TokenStream` of the form `var1: type1, var2: type2:, ...` from a list of `Variables` using owned types
/// Can be used for struct members.
pub(crate) fn generate_member_list(params: &[Variable]) -> TokenStream {
    let param_names = params.iter().map(|p| format_ident!("{}", change_case(&p.name, IdentCase::LowerSnakeCase)));
    let param_types = params.iter().map(|p| skylite_type_to_rust_owned(&p.typename));
    quote! {
        #(#param_names: #param_types),*
    }
}
...
/// Generates a list of statements of the form `let <name> = <type>::deserialize(decoder);`.
...
/// Generates a comma-separated list of expressions compatible with a parameter list
/// generated by `generate_param_list`.
/// Can be used as a building block for decode functions.
pub(crate) fn generate_argument_list(args: &[Variable]) -> TokenStream {
    let expressions = args.iter()
        .map(|v| {
            let ident = format_ident!("{}", change_case(&v.name, IdentCase::LowerSnakeCase));
            match v.typename {
                Type::U8 | Type::U16 | Type::U32 | Type::U64
                | Type::I8 | Type::I16 | Type::I32 | Type::I64
                | Type::F32 | Type::F64 | Type::Bool | Type::Tuple(_) => quote!(#ident),
