...
use crate::actors::{Actor, InstanceId, TypeId};
...
impl<'scene, P: SkyliteProject> ActorIterator<'scene, P> {
    pub fn _private_new<'s>(
        main: &'s [Box<dyn Actor<P = P>>],
        extras: &'s [Box<dyn Actor<P = P>>],
    ) -> ActorIterator<'s, P> {
        ActorIterator {
    ...
    /// Filters the iterator to only include the actors of a particular type.
    /// The items of the returned iterator will already be converted to that
    /// actor type.
...
impl<'scene, P: SkyliteProject> ActorIteratorMut<'scene, P> {
    pub fn _private_new<'s>(
        main: &'s mut [Box<dyn Actor<P = P>>],
        extras: &'s mut [Box<dyn Actor<P = P>>],
    ) -> ActorIteratorMut<'s, P> {
        ActorIteratorMut {
...
impl<'scene, P: SkyliteProject, Filter: TypeId + InstanceId + 'scene> Iterator
    for ActorIteratorFiltered<'scene, P, Filter>
{
...
impl<'scene, P: SkyliteProject, Filter: Actor + 'scene> Iterator
    for ActorIteratorFilteredMut<'scene, P, Filter>
{
...
/// A `Scene` is a single screen or context of a project, e.g. an individual
/// level or menu. There are two lists of [`Actors`][Actor] which make up a
/// `Scene`:
/// - The main actors, or just 'actors' are fixed for each scene. These are the
///   actors which are accessible to plays.
/// - The extra actors, or just 'extras', which can be added to or removed
///   during a scene's lifetime. These actors are not accessible to plays.
///
/// A `Scene` will update each actor when the `Scene` itself is updated, and
/// render each actor when it is rendered itself.
pub trait Scene {
    type P: SkyliteProject;
    type ActorNames: Into<usize>
    where
        Self: Sized;

    #[doc(hidden)]
    fn _private_decode(decode: &mut dyn Decoder) -> Self
    where
        Self: Sized;
    #[doc(hidden)]
    ...
    #[doc(hidden)]
    fn _private_get_named_actor_mut_usize(&mut self, name: usize) -> &mut dyn Actor<P = Self::P>;
    ...
    fn get_named_actor(&self, name: Self::ActorNames) -> &dyn Actor<P = Self::P>
    where
        Self: Sized;

    /// Returns a mutable reference to a named actor in the `Scene`, or `None`
    /// if the name does not exist.
    fn get_named_actor_mut(&mut self, name: Self::ActorNames) -> &mut dyn Actor<P = Self::P>
    where
        Self: Sized;
}
...
///
/// This trait is implemented automatically for a type by `skylite_project!`.
/// The type that implements this trait will be an enum with a variant for each
/// scene in the project, with fields for each parameter for a scene.
///
/// ```ignore
...
pub mod _private {
    ...
    pub fn render_scene<'scene, P: SkyliteProject>(
        scene: &'scene dyn Scene<P = P>,
        ctx: &mut DrawContext<P>,
    ) {
        ...
        scene
            .iter_actors(IterActors::All)
            .for_each(&mut insert_by_z_order);
    ...
    impl<P: SkyliteProject> Scene for DummyScene<P> {
        ...
        fn _private_decode(_decode: &mut dyn skylite_compress::Decoder) -> Self
        where
            Self: Sized,
        {
            unimplemented!()
        }
        fn _private_update(&mut self, _controls: &mut crate::ProjectControls<Self::P>) {
            unimplemented!()
        }
        fn _private_render(&self, _ctx: &mut DrawContext<Self::P>) {
            unimplemented!()
        }
        fn _private_get_named_actor_mut_usize(
            &mut self,
            _name: usize,
        ) -> &mut dyn Actor<P = Self::P> {
            unimplemented!()
        }
        fn iter_actors(&self, _which: IterActors) -> super::ActorIterator<Self::P> {
            unimplemented!()
        }
        fn iter_actors_mut(&mut self, _which: IterActors) -> super::ActorIteratorMut<Self::P> {
            unimplemented!()
        }
        fn add_extra(&mut self, _extra: Box<dyn Actor<P = P>>) {
            unimplemented!()
        }
        fn remove_current_extra(&mut self) {
            unimplemented!()
        }
        fn get_named_actor(&self, _name: Self::ActorNames) -> &dyn Actor<P = Self::P>
        where
            Self: Sized,
        {
            unimplemented!()
        }
        fn get_named_actor_mut(&mut self, _name: Self::ActorNames) -> &mut dyn Actor<P = Self::P>
        where
            Self: Sized,
        {
            unimplemented!()
    ...
    pub fn replace_scene<P: SkyliteProject + 'static>(
        src: P::SceneParams,
        dst: &mut Box<dyn Scene<P = P>>,
    ) {
